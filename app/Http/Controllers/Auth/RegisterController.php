...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function apiRegister(){
        $isSuccessed = 0;
        $imageName = "androidUser_" . time() . ".jpeg";
        try {
            if(!User::where('email',$_POST['email'])->first()){
                //  ready for registration
                file_put_contents("storage/user_images/".$imageName,base64_decode($_POST['image']));
                $user_data = [
                    'name' => $_POST['nameSurname'],
                    'email' => $_POST['email'],
                    'password' => Hash::make($_POST['password']),
                    'user_opg' => $_POST['familyFarm'] ? $_POST['familyFarm']:"Unset",
                ...
                if($_POST['userType'] === "Prodavač"){
                    $final_data = array_merge($user_data,['user_type'=>'seller']);
                    User::create($final_data);
                }
                else{
                    $final_data = array_merge($user_data,['user_type'=>'buyer']);
                    User::create($final_data);
